...
//        intersection.exit.xyz -= uFire.smokeDomain.center;
//        intersection.exit.xyz += uFire.smokeDomain.radius;
//        intersection.exit.xyz /= 2.0 * uFire.smokeDomain.radius;
//
//        gl_FragColor = vec4( intersection.exit.xyz, 1.0 );

//        intersection.enter.xyz -= uFire.smokeDomain.center;
//        intersection.enter.xyz += uFire.smokeDomain.radius;
//        intersection.enter.xyz /= 2.0 * uFire.smokeDomain.radius;
//
//        gl_FragColor = vec4( intersection.enter.xyz, 1.0 );
...
//    gl_FragColor = vec4( abs( normalize( ray.origin.camera.xyz ) ), 1.0 );
...
//    gl_FragColor = vec4( ray.origin.texture.xy, 0.0, 1.0 );

    ray.origin.atlas          = vec3( convertToAtlasSpace( ray.origin.texture.xy, uFire.frame, uFire.resolutionXY ), 1.0 );
    ray.end.atlas             = vec3( convertToAtlasSpace( ray.end.texture.xy,    uFire.frame, uFire.resolutionXY ), 1.0 );
...
//        gl_FragColor = vec4( currentTexCoord.xy, 0.0, 1.0 );
...
//    vec3 captureCamPosition = normalize( uCaptureCameras[0].viewInv[3].xyz );
//
//    gl_FragColor = vec4( captureCamPosition, 1.0 );
...
//    gl_FragColor = vec4( intersection.enter.xyz, 1.0 );
    ...
    int  steps         = 64;
    vec2 newTexCoords  = convertToAtlasSpace( vTexCoords, uFire.frame, uFire.resolutionXY );
...
//                }
//
//        gl_FragColor = vec4( newTexCoords, 0.0, 1.0 );
//
